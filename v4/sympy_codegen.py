...
{equation_code}        }});"""
        ...
        # Generate Python expression comments
        python_expressions = []
        for eq in equations:
            python_expressions.append(f"//   {eq}")
        python_expr_comments = "\n".join(python_expressions)
...
// HPX runtime management - separated for performance
static bool hpx_runtime_initialized = false;

extern "C" void init_hpx_runtime()
{{
    if (!hpx_runtime_initialized) {{
        int argc = 0;
        char *argv[] = {{ nullptr }};
        hpx::start(nullptr, argc, argv);
        hpx_runtime_initialized = true;
    }}
}}

extern "C" void finalize_hpx_runtime()
{{
    if (hpx_runtime_initialized) {{
        hpx::post([](){{ hpx::finalize(); }});
        hpx::stop();
        hpx_runtime_initialized = false;
    }}
}}
...
{{
    ...
    return 0;  // Don't call hpx::finalize() here anymore
}}
...
{{
    ...
    // Runtime stays alive for subsequent calls
}}
"""
...
"""
    ...
    def _compile_cpp_code(self, cpp_code_pair: Tuple[str, str], func_name: str) -> str:
        ...
        if not cpp_changed and not cmake_changed and os.path.exists(so_file):
            print(f"HPX multi-dimensional kernel for {func_name} is up to date, skipping compilation...")
        ...
        # Check VERBOSE environment variable for debug output
        verbose_mode = os.environ.get('VERBOSE', '0') == '1'
        capture_output = not verbose_mode  # Show output when VERBOSE=1
        
        try:
            subprocess.run(cmake_cmd, cwd=build_dir, check=True, capture_output=capture_output, text=True)
            subprocess.run(["make", "-j"], cwd=build_dir, check=True, capture_output=capture_output, text=True)
    ...
    def _create_python_wrapper(self, so_file: str, func_name: str, equations: List[sp.Eq]):
        ...
        # Set up HPX runtime management functions
        init_hpx = getattr(lib, 'init_hpx_runtime')
        finalize_hpx = getattr(lib, 'finalize_hpx_runtime')
        init_hpx.argtypes = []
        init_hpx.restype = None
        finalize_hpx.argtypes = []
        finalize_hpx.restype = None
        
        # Get function signature info
        vector_vars, scalar_vars, result_vars, stencil_info, array_dims = self._analyze_equations(equations)
        
        # Set up atexit handler for HPX runtime cleanup
        import atexit
        atexit.register(finalize_hpx)
        ...
        # Add cleanup function as attribute for manual control
        wrapper.finalize_hpx = finalize_hpx
